...
La versión se calcula automáticamente a partir de los tags de Git, no a mano (anti-patrón §4.8 "versionado manual"). La herramienta de auto-versioning está anclada a `SOLUTION-INTAKE §17 P.7`, que fija la capacidad y deja la elección puntual abierta a Sprint 0 entre las dos candidatas declaradas allí.
...
- Elección puntual (cerrada): se adopta **Nerdbank.GitVersioning (nbgv)**, una de las dos candidatas de `§17 P.7`, por baja fricción de tooling — se integra como paquete de build privado (`PackageReference` con `PrivateAssets=all` en `Directory.Build.props`), sin workflows ni binarios extra, e inyecta la versión en los ensamblados (`AssemblyInformationalVersion`/`FileVersion`) de toda la solución. La versión base vive en `version.json` (raíz: `version` = `1.0`, `assemblyVersion.precision` = `major`, `publicReleaseRefs` = `main` y tags `v<X.Y>`). En `main` y en tags `v*` produce versiones públicas (sin sufijo de altura de commit); en ramas de feature produce prerelease (`X.Y.Z-g<commit>`). El pipeline de publicación deriva la versión con la action `dotnet/nbgv` (variable `NBGV_NuGetPackageVersion`) en vez de parsear el tag a mano. Esta operacionalización se registra en el control de cambios de este documento y en el README del repositorio (categoría 10 colapsada, ADR-11), sin cambiar el resto de la estrategia.
...
- La herramienta de auto-versioning se ancla a `§17 P.7`; la elección puntual se cerró en Nerdbank.GitVersioning (nbgv) — ver §3 y el control de cambios.
...
| 1.0 | 2026-06-21 | Operacionalización (categoría 09): la elección puntual entre GitVersion y Nerdbank.GitVersioning se **difiere a Sprint 0** (conforme a §3 y `§17 P.11`), por no introducir fricción de tooling en v1. Mientras tanto, el pipeline de publicación (`.github/workflows/publish.yml`) **deriva la versión del tag SemVer** `v<X.Y.Z>[-sufijo]` que dispara el release (precedencia de prerelease por sufijo `-rc.N`/`-beta.N`/`-alpha.N`), que es el comportamiento esperado de cualquiera de las dos herramientas y no condiciona la decisión. Al cerrarse la elección en Sprint 0 se agrega el archivo de configuración de la herramienta y se registra acá, sin cambiar el resto de la estrategia. |
| 1.0 | 2026-06-21 | **Cierre de la decisión diferida (§3):** se adopta **Nerdbank.GitVersioning (nbgv) 3.10.85** como herramienta de auto-versioning, por baja fricción. Se agrega `version.json` en la raíz (base `1.0`, `assemblyVersion.precision: major`, `publicReleaseRefs` = `main` y tags `v<X.Y>`) y el `PackageReference` privado (`PrivateAssets=all`) en `Directory.Build.props`, que inyecta la versión SemVer en los ensamblados de toda la solución (también el proyecto e2e, que comparte el props; verificado: no lo rompe). El `publish.yml` deja de parsear el tag a mano y deriva la versión con la action `dotnet/nbgv` (`NBGV_NuGetPackageVersion`), manteniendo el disparo por tag `v*`; el valor se refleja en el nombre del zip, el SBOM, el título del release y el flag `--prerelease`. Se añade `fetch-depth: 0` a los checkouts de los jobs `e2e` y `supply-chain` de `ci.yml` (nbgv requiere historial git completo). Comportamiento: `main`/tag `v*` ⇒ versión pública sin sufijo; rama de feature ⇒ prerelease `X.Y.Z-g<commit>`. El resto de la estrategia (SemVer, Conventional Commits, branching, canales, deprecation) no cambia. |
